...
Para todos os três, o objetivo é **chegar ao mesmo estado**: projeto com `.github/agents/geral-robusto/` (este pacote) + `docs/` no padrão definido aqui.
...
1. Este arquivo vence regras gerais do pacote `.github/agents/geral-robusto/`
2. **Exceções inegociáveis** (ver `.github/agents/geral-robusto/01-nucleo.md` seção 1.3):
...
Conforme [módulo 11, seção 9](../padroes/11-arquitetura-e-pastas.md#9-estrutura-padrão-de-docs):
...
3. **Use o formato padrão** dos requisitos: [`templates/38-requisitos.md`](../templates/38-requisitos.md) (RF/RN/RNF, com prioridade, status e rastreabilidade requisito↔tarefa↔ADR)
...
Esta pasta contém documentação **anterior à inicialização do projeto no padrão `.github/agents/geral-robusto/`** (data: 2026-05-13).
...
Arquive apenas a documentação **conceitual** que não casa com o padrão `.github/agents/geral-robusto/`.
...
(diverge do padrão do `.github/agents/geral-robusto/` em `stores/` e `api/`; documentado em ADR-001)
...
2. Exceções inegociáveis do `.github/agents/geral-robusto/01-nucleo.md` aplicam
```
...
|Forçar estrutura do `.github/agents/geral-robusto/` em projeto que diverge|Refatoração não-pedida|Documente a divergência via ADR|
