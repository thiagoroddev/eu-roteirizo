...
O pacote `.github/agents/geral-robusto/` é **portátil entre projetos**. Para fazer o "salto" entre regras genéricas e contexto específico, existe **um único ponto de entrada**: `docs/contexto-projeto-ai.md`.
...
⚠️ **Diverge do padrão do pacote `.github/agents/geral-robusto/`** em alguns pontos. Documentado em [ADR-001](./arquitetura/ADR/ADR-001.md).
...
2. **Núcleo do pacote** (`.github/agents/geral-robusto/01-nucleo.md`) vence nas **3 exceções inegociáveis**:
   - Confirmação antes de ações destrutivas
   - Proibição de `any` sem justificativa
   - "Código é a verdade primária"
3. **Demais módulos** de `.github/agents/geral-robusto/`

Detalhes da hierarquia: [`.github/agents/geral-robusto/01-nucleo.md` seção 1.3](../.github/agents/geral-robusto/01-nucleo.md#13-hierarquia-de-regras-resolução-de-conflito).
...
- **Data:** 13/05/26
- **Por:** Inicialização via módulo 26 do pacote `.github/agents/geral-robusto/`
...
[Se segue o padrão de `.github/agents/geral-robusto/11-arquitetura-e-pastas.md`, basta dizer "Padrão do pacote."]
...
A estrutura segue o padrão do pacote `.github/agents/geral-robusto/11-arquitetura-e-pastas.md` sem divergências.
```
...
Este `contexto-projeto-ai.md` foi criado em DD/MM/AA via [módulo 26 (inicialização)](../.github/agents/geral-robusto/processos/26-inicializacao-projeto.md). O projeto **já existia** antes da adoção do pacote `.github/agents/geral-robusto/`.
...
## Estrutura
Padrão do pacote `.github/agents/geral-robusto/`.
