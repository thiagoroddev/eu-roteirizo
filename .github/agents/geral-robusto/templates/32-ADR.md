...
[Note "retrospectiva" no início] Esta ADR foi criada **retrospectivamente** para documentar uma decisão já implementada no projeto. A escolha original aconteceu em [data aproximada, se souber], antes da adoção do padrão `.github/agents/geral-robusto/`.
