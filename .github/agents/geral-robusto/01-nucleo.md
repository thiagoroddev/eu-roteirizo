...
> **Este arquivo é sua base permanente.** Ele é carregado em toda interação. Tudo aqui é lei. Tudo que não está aqui é opcional ou contextual e vive nos demais módulos da pasta `.github/agents/geral-robusto/`.
...
A pasta `.github/agents/geral-robusto/` contém instruções de **comportamento do agente** - como a IA deve pensar e agir. Ela é **separada** das instruções do projeto:

|Pasta|O que contém|Quando muda|
|---|---|---|
|`.github/agents/geral-robusto/`|Como a IA se comporta (este pacote)|Raramente, evolui com a prática|
...
- Consulte a [tabela de carregamento](#9-tabela-de-carregamento-de-módulos) ao final deste arquivo.
...
2. .github/agents/geral-robusto/01-nucleo.md           (este arquivo)
3. Demais módulos de .github/agents/geral-robusto/
```
...
Nem toda tarefa precisa do mesmo processo. Use o modo apropriado (ver [seção 4](#4-modos-de-cerimônia)). Corrigir um typo não exige ADR. Mudar a estrutura de pastas, sim.
...
Conforme o modo de cerimônia (ver [seção 4](#4-modos-de-cerimônia)).
...
|Levantar/documentar requisitos (RF/RN/RNF)|[`processos/26-inicializacao-projeto.md`](./processos/26-inicializacao-projeto.md) (extração do código) + [`templates/38-requisitos.md`](./templates/38-requisitos.md) (formato)|
...
> **Lembrete final:** este arquivo é a fundação. Os demais módulos refinam, exemplificam e detalham - mas nada aqui pode ser ignorado, exceto pelas exceções formais descritas na [seção 1.3](#13-hierarquia-de-regras-resolução-de-conflito).
