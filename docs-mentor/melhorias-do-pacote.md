...
- **13/09/26 00:25** · task finalizar (v0.6.0) nao consegue ler caminho com hifen no plano.muda: cmd-tarefa.ts extrai o arquivo com linha.split(/[\s:—-]/)[0], que corta no hifen, entao .mentor/scripts/cmd-doctor.ts vira .mentor/scripts/cmd e a checagem de arquivos fantasma (AUD-001-B05) acusa arquivo declarado como nao declarado. Medido na TASK-CHORE-017 (cmd-doctor.ts, cmd-fila.ts, cmd-tarefa.ts): o contorno foi comecar a linha por um sufixo sem hifen. Sugestao: extrair o caminho ate o primeiro espaco seguido de separador (" - ", ": ", " — "), nao em qualquer hifen. Inconsistencia vizinha: o hook de pre-push trata .mentor/ como nao-codigo (arquivoEhCodigo), mas o finalizar nao ignora .mentor/ e exige declarar os 12 arquivos do pacote numa atualizacao do proprio mentor.
