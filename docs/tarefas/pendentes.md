...
> - **Gate de cada tarefa:** `npm run typecheck` + `npm run test` verdes antes de concluir (rotular APROVADO/FALHOU/NÃO EXECUTADO). ⚠️ `npx tsc --noEmit` **não** serve: sai 0 sem checar nada neste tsconfig (TASK-BG-010).
...
| TASK-BG-009 | **Teste INSTÁVEL (flaky) por esperar em relógio de parede.** `osm.test.ts > "omits stats when the request fails"` mocka HTTP 429 e chama `fetchRoadGraph` **sem injetar o override `sleep`**; desde que a BG-008 tornou 429 retryable com backoff real, a espera soma ~4,5 s contra timeout de 5 s do Vitest — **passa ou falha conforme a carga da máquina** (observado verde e vermelho em execuções consecutivas em 27/07). ⚠️ Corrigido o diagnóstico inicial, que o classificara como falha determinística. Corrigir passando `{ sleep: async () => {} }` (o override já existe, o teste é que não usa) e varrer irmãos no mesmo caso | Standard | Importante | Normal | P/P | - | TASK-BG-008 | `[ ]` | 27/07/26 11:26 |
| TASK-DOC-010 | **Eliminar travessões (`—`) da documentação viva.** Regra criada no núcleo §6.4 (v4.2) vale daqui pra frente; esta tarefa limpa o legado. **944 ocorrências**: `docs/` vivos (865), `.github/agents/geral-robusto/` (56), `README.md` (23). ⚠️ **NÃO é busca-e-substitui**: o travessão liga orações e apagá-lo quebra a frase (`Sem backend — tudo é local` vira `Sem backend tudo é local`). Cada caso escolhe vírgula, dois-pontos, parênteses, ponto final ou conector, pela tabela do §6.4. Sugestão de fatiar por arquivo, do mais visível ao menos: `README.md` (23) → `contexto-projeto-ai.md` (32) → ADRs → requisitos → `pendentes.md` (64) → rascunhos (o maior volume e o menos lido). **Fora do escopo por decisão do humano:** `docs/tarefas/concluidas/` (1.688, registros imutáveis) e `src/` (853, comentários e `UI_LABELS`) | Standard | Importante | Normal | M/G | - | núcleo §6.4 | `[ ]` | 27/07/26 19:15 |
| TASK-CHORE-009 | **Comprimir as capturas de `docs/imagens/`** — 7 PNGs somam ~4 MB (as do mapa, ~1 MB cada). Infla o clone e deixa o README lento justamente em conexão ruim, que é o cenário de quem abre no celular. Avaliar `oxipng`/`pngquant` ou reduzir o `deviceScaleFactor` da recaptura | Light | Desejável | Normal | P/P | TASK-DOC-007 | - | `[ ]` | 27/07/26 16:14 |
| TASK-DOC-008 | **Divergência entre `UI_LABELS.SUMMARY.VIEW_MAP` e a tela.** A constante vale `"Ver Original"`, mas o Sumário renderiza `"Ver no Mapa"` (descoberto ao automatizar o clique na DOC-007). Ou a constante está morta, ou há rótulo hardcoded fugindo da ADR-001 — nenhum teste afirma o texto desse botão | Light | Desejável | Normal | P/P | - | ADR-001, TASK-REF-017 | `[ ]` | 27/07/26 16:14 |
| TASK-CHORE-008 | **Remover `public/sw.js` — service worker fantasma.** Nada o registra e o `vite-plugin-pwa` **sobrescreve** `dist/sw.js` com o do Workbox no build, então o arquivo nunca é servido. Manter um SW morto no repo convida alguém a editá-lo achando que tem efeito (achado na CHORE-007) | Light | Desejável | Normal | P/P | - | - | `[ ]` | 27/07/26 11:27 |
