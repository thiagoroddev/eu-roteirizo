...
| TASK-BG-009 | **Suíte vermelha por teste que espera em relógio de parede.** `osm.test.ts > "omits stats when the request fails"` mocka HTTP 429 e chama `fetchRoadGraph` **sem injetar o override `sleep`**; desde que a BG-008 tornou 429 retryable com backoff real (1,5 s → 3 s → …), o teste estoura o timeout de 5 s do Vitest. Corrigir passando `{ sleep: async () => {} }` (o hook já existe, o teste é que não usa) e varrer se há irmãos no mesmo caso | Standard | Importante | Normal | P/P | - | TASK-BG-008 | `[ ]` | 27/07/26 11:26 |
