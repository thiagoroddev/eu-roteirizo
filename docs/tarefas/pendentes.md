...
- **REQ/ADR/DT:** RF-21..28, RF-32, RF-33, RF-40; ADR-009; `fluxo-roteirizacao.md` (spec vigente, §4 passos, §6 ordem a pé, §10 decisões, §10.10 painéis por contexto); `docs/design/arvore-componentes-mapa.md` (matriz modo×slot, telas 5 a 9)
...
### ~~TASK-RF-006.6~~ - Parada firmada: visualização + edição no painel (telas 5 a 7)
...
- **Status:** ✅ **CONCLUÍDA (18 a 19/07)**, **.1** motor + config + split (18/07) e **.2** config global no ⚙️ + regra por tipo/complemento (19/07). Ver índice de concluídas.
- **Modo:** Standard
...
**Critérios de aceite (RF-007):** ✅ os três tempos aparecem decompostos e somados corretamente; funções puras cobertas por teste; config global persiste. **RF-007 CONCLUÍDA (.1 + .2, 18 a 19/07).**
**Dependências novas:** nenhuma.
