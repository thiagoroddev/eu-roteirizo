...
| TASK-CHORE-010 | **Gate de typecheck do pacote do agente aponta para um comando que não checa nada.** `01-nucleo.md` §10 e a tabela de gates mandam usar `npx tsc --noEmit`, que neste projeto lê **zero arquivos** (tsconfig raiz é solution-style; `tsc` só segue project references com `-b`). Trocar por `tsc -b` em `.github/agents/` e considerar um script `npm run typecheck` para não depender de quem lembra o `-b`. Docs do projeto já corrigidas na BG-010 | Standard | **Crítico** | Normal | P/P | TASK-BG-010 | - | `[ ]` | 27/07/26 18:30 |
