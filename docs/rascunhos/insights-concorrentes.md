...
| Base **Google Maps** | um dos apps (pins azuis) | — | Paga por *map load* (SDK Google) | ⚠️ Evitar — somos OSM/Leaflet |
...
| **"Receba bônus diário"** (recompensa/ad diário) | Tour Entregas | **Estratégico** — monetização | — | 🤔 (confirma o ad recompensado da nossa análise) |
| **"Laço"** (botão de config cortado "Lac…") | app Google-based | Investigar — pode ser laço/seleção de paradas | — | 🔎 investigar no próximo print |
...
> A maioria repete padrões dos lotes 1–2 (Google-based). Abaixo só o que é **novo e relevante**.
...
| **Preço: R$ 19,99/mês** (plano anual) | banner laranja | **Estratégico** — referência de mercado | — | 💰 intel |
...
| **"Não otimize, navegue como adicionado"** (respeitar ordem manual) | "Nova Rota" | **Estratégico** — valida o nosso diferencial (ordem manual) | — | ✅ (confirma) |
...
| Roteamento via **GraphHopper API** | rodapé "distribuído por" | — | Eles pagam **API hospedada**; nós não (A\* local) | 🧱 nota |
