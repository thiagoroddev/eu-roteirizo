...
- **Preço como cunha:** concorrentes cobram uma ordem de grandeza a mais (Circuit/Spoke US$10 a 20, RoadWarrior US$49/mês), e os free tiers param em 8 a 10 paradas.
...
| **PMTiles** | Formato do tileset (arquivo único, range requests) | n/a | 🔭 |
| **Protomaps** | Basemap vetorial (extrai região do planeta) | n/a | 🔭 ADR-007 (provedor ainda aberto: §4.6) |
...
> Consolidação das ADRs 001 a 010 e das DTs. **A ADR continua sendo a fonte da verdade**; aqui fica só a justificativa central, para leitura corrida.
...
3. **Zoom.** O bloqueio de `z < 14` impede a visão geral da rota; a decisão é liberar até ~z10 a 11 e deixar o cache absorver.

**E por que vetor, e não raster melhor:**

- **É mais barato, não mais caro:** ~**4 requisições por visualização** contra **10 a 16 do raster**, com payloads menores.
...
**Provedores comparados:** MapTiler (~100 k tiles/mês grátis, editor de estilo, bom para começar) · Stadia (free tier, MapLibre-friendly) · **Mapbox evitado** (GL JS v2+ proprietário e cobra **por *map load***) · OpenMapTiles+TileServer GL em VPS (~R$250 a 500/mês, "mais ops, provavelmente dispensável") · **Protomaps/PMTiles em R2** como destino.
...
| Pages | US$ 0 · bandwidth **ilimitado** · 500 builds/mês · **25 MiB/arquivo** | n/a |
...
| Google / Mapbox Temporary | n/a | n/a | ❌ Proíbem |
...
| Magalu Cloud BV4-8-40 | 4 vCPU / 8 GB / 40 GB | n/a | R$ 170 |
...
**O ponto frágil não é a escala grande: é a pequena.** A partir de ~3.000 ativos o custo por usuário some (R$ 0,06 a 0,09). Quem aperta é o **piso fixo** do geocoding: a R$ 120/mês com 300 ativos seria preciso converter **9,4%**, muito acima do 0,5% que a documentação assume.
