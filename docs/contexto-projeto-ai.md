...
| shadcn/ui | n/a | **biblioteca de UI padrão** (Radix + `cva` + `cn`); componentes em `src/components/ui/`. Ver **[ADR-004](./arquitetura/ADR/ADR-004.md)** |
...
| ESLint 9 + Prettier | n/a | `npm run lint` |
...
- **Roteamento local (ADR-002):** grafo dirigido sobre dados OSM/Overpass + A* próprio, com **nenhuma API de roteirização paga** (RNF-03/13). Mão única = aresta ausente; cache offline do grafo.
...
- **xlsx fixado na versão da CDN (0.20.3)** por segurança.
- **Imports por alias:** `@/`, `@assets/`, `@components/`, `@utils/`.
- **UI / Design System (ADR-004):** shadcn/ui; componentizar e DRY são lei; nada de cor/classe hardcoded fora dos tokens; primitivos complexos via Radix, sem reinventar.
...
- **Smoke no aparelho é gate de UI:** suíte verde não prova conforto visual/gesto (lição das RF-006.4.19/.4.20). Deploy do protótipo: `npm run deploy:test` → `https://teste-prototipo.pages.dev` (recarregar 2×).
...
- ❌ **Execução da rota** (RF-009), **auto-roteirizar** (RF-012) e **export/import de roteiro** (RF-013) seguem pendentes; o botão "Importar roteiro (.json)" da HOME é stub.
...
- ✅ **Deploy de PROTÓTIPO no Cloudflare Pages** (TASK-CHORE-013, 07/09/26; projeto renomeado p/ `teste-prototipo`): `npm run deploy:test` publica em **https://teste-prototipo.pages.dev**, URL fixa p/ smokes no celular (HTTPS → GPS/PWA funcionam; recarregar **2×** p/ o SW `autoUpdate` assumir). ⚠️ **É protótipo, NÃO o lançamento**: `X-Robots-Tag: noindex` via `public/_headers` (remover esse arquivo quando existir deploy de produção, que é outra decisão). A **URL é divulgada** no README e em portfólio, só não é indexada. Mesma conta Cloudflare do tile worker. Instruções em [`docs/README.md`](./README.md) §"Testar no celular".
...
- **ADRs 001 a 010:** [`docs/arquitetura/ADR/`](./arquitetura/ADR/), cobrindo idioma/i18n · roteamento OSM local · app shell/router · shadcn · remoção Correios · identidade visual (rev. Neon Flux) · basemap/tiles · marcadores SVG · interop dos modos · fonte da malha viária
...
- **Fase:** visualizador (modo Original) **estável e polido**; **Meu roteiro** com o épico RF-006 **fechado em 20/07/26** (entrar no modo, início por GPS/toque/endereço, grafo OSM lazy, criar/editar parada por raio, zoom por contexto, painel por modo, gestos da âncora, edição plena, traçado real pelas ruas, visão geral e navegação até a âncora).
- **Fila:** `RF-009` (execução da rota, XG/Strict, plano do épico aprovado) → `RF-012` (auto-roteirizar) → `RF-013` (export/import JSON). Detalhe no topo de [`pendentes.md`](./tarefas/pendentes.md).
- **Testes:** **774/774** em 72 arquivos (27/07/26); gates por tarefa: `npm run typecheck` + `eslint` + `vitest` + `build`, rotulados no registro. ⚠️ **`npx tsc --noEmit` não serve como gate aqui**: o `tsconfig.json` da raiz é solution-style e o comando sai 0 sem ler arquivo nenhum (TASK-BG-010); o script `typecheck` roda `tsc -b`, que é o que de fato checa.
