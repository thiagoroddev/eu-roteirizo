...
| RF-12 | Ao tocar um marcador, o usuário vê os dados da entrega no **painel inferior persistente** (`MapPanel`/`StopItemList` na tela `/mapa`: item expande com bairro/CEP/tipo, pacotes SPX/seq e link Maps — fluxo-modo-original §6; tooltip segue escapado; `AddressSheet` sobrevive só no modal legado até a TASK-REF-011) | SHOULD | ✅ | `components/map/panel/` (MapPanel/StopItemList/StopItem), `pages/MapPage.tsx`; legado: `components/map/AddressSheet.tsx` | TASK-BG-003, TASK-RF-022.6, TASK-RF-023.2–.5 |
