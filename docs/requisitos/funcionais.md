...
| RF-07 | O usuário pode selecionar qual rota visualizar (multi-rota), **pelos chips do card na aba Rotas** (o seletor inline da HOME foi removido) | MUST | ✅ | `pages/RoutesPage.tsx`, `components/manifests/ManifestCard.tsx` | TASK-RF-022.3, TASK-REF-011 |
| RF-08 | O usuário pode localizar a rota pelo código AT (`Planned AT`), **pelo filtro da aba Rotas** (estreita cards e chips; a busca inline da HOME foi removida) | SHOULD | ✅ | `pages/RoutesPage.tsx` (filtro), `components/manifests/ManifestCard.tsx` | TASK-RF-022.3, TASK-REF-013, TASK-REF-011 |
...
| RF-12 | Ao tocar um marcador, o usuário vê os dados da entrega no **painel inferior persistente** (`MapPanel` na tela `/mapa`: card do selecionado + drill-down com pacotes/complementos/SPX e link Maps; fluxo-modo-original §6; tooltip segue escapado; `AddressSheet` **aposentado** na REF-011) | SHOULD | ✅ | `components/map/panel/` (MapPanel/StopItem/StopItemList), `pages/MapPage.tsx` | TASK-BG-003, TASK-RF-023, TASK-REF-011 |
| RF-13 | O usuário pode abrir o mapa em tela cheia: **é a própria tela `/mapa`** (FocusShell; o modal legado foi removido) | COULD | ✅ | `pages/MapPage.tsx`, `components/RouteMap.tsx` | TASK-RF-022.5, TASK-REF-011 |
...
| RF-23 | Criar parada a partir de um endereço, **sugerindo** os que estão dentro do raio (o usuário escolhe quais entram: toque no mapa ou painel; círculo tracejado; raio ajustável de 10 a 200 m) | MUST | ✅ | fluxo §4/§8 | TASK-RF-006.4 ✅ |
| RF-24 | Ajustar a parada manualmente (adicionar/remover endereços), **no RASCUNHO ✅ (toggle livre + incorporar órfão + aviso suave RN-17); editar parada FIRMADA = .6** | MUST | 🟡 | fluxo §5/§9; telas 5/6 | TASK-RF-006.4 ✅ (rascunho), TASK-RF-006.6 (firmada) |
...
| RF-29 | Traçar o caminho de veículo pela rua real **entre paradas do veículo**, respeitando mão única, com km: **motor (TASK-RF-005) + UI (TASK-RF-006.7): linha contínua start→âncoras + km real nos totais** | MUST | ✅ | ADR-002; fluxo §6; `utils/routing/` | TASK-RF-005 ✅, TASK-RF-006.7 ✅ |
| RF-30 | Mostrar a distância a pé pelas ruas até o alvo selecionado (A* por alvo; reta como fallback): **SUGESTÃO (TASK-RF-006.3) + circuito da parada e km a pé nos totais (TASK-RF-006.7); falta só o POR-PERNA na lista (TASK-RF-006.10)** | SHOULD | 🟡 | fluxo §6; ADR-002; `utils/routing/` | TASK-RF-005 ✅, TASK-RF-006.3 ✅, TASK-RF-006.7 ✅ |
