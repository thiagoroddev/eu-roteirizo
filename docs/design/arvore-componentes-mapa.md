...
> **Doc de design da TASK-RF-023** (fase Original). Mapeia os componentes da tela do mapa e do painel inferior a partir das telas de média fidelidade (`prototipos/telas-media-fidelidade/`, telas 5 a 9) e das specs textuais (`fluxo-modo-original.md`, que decide no Original; `fluxo-roteirizacao.md`, que decide no Meu roteiro). As imagens **ilustram estrutura e informação**; cores/estética = tokens do tema (ADR-006 rev. Neon Flux; personalização via `theme.css`).
>
> **Decisões do humano (05/07/26) já incorporadas:** painel **sempre visível** (colapsado = nº da parada + endereço); **`vaul`** (Drawer shadcn) para gesto/snap points; **nunca existe "nenhuma parada selecionada"** (abre com a **menor parada**; sempre mostra a **última selecionada**; clique fora não esvazia); **fase Original primeiro** (só dados da planilha, **sem numeração nova**, ou seja, Stop/Sequence), Meu roteiro depois preenchendo **slots**.
...
└── MapPanel 🆕                         bottom sheet persistente (vaul: snaps colapsado/meio/cheio,
    ...
    │   ├── [slot] actions 🔮           grade de botões contextuais (telas 5 a 9: Editar/Iniciar/
...
| Título | `Parada {Stop}` + endereço representante | n/a (corpo = só o card "Próxima parada sugerida") | card "Roteiro em construção" | `Parada N · Veículo (âncora)` | `Parada N · 2º endereço` | `Parada N · Veículo` | próxima entrega |
...
| actions | n/a (vazio) | "Criar parada" + ícone mapa (card) | idem + "Ver no mapa"/parada + "Redefinir início" (parada 0) | `Editar Parada · Iniciar rota` | `Remover/Adicionar/Tornar âncora/Inverter` ou `Mover/Resetar âncora` | `Salvar parada · Editar âncora/raio` | `Abrir GPS · Concluir` |
| banner | n/a | n/a | n/a | n/a | n/a | "raio engloba N candidatos" | n/a |
| Itens: número | **Sequence planilha** | n/a | **parada 0 = início** (glifo azul) + PanelTitle por parada; drill-down com ordinais | ordinal 1º/2º + item âncora | idem + drag | idem | ordem do roteiro |
...
| footer | n/a | n/a | n/a | n/a | `Salvar alterações` | `Salvar alterações` | `Concluir entrega` |
...
## 7. Recorte da fase Original (TASK-RF-023.2 a .5)
...
- **Data:** 05/07/26, criação (TASK-RF-023.1), a partir das telas 5 a 9 + fluxos + decisões do humano de 05/07.
